...
- **An unsupported `WHERE`/`HAVING`/`JOIN ON` clause is now a hard error, not silent data loss**: the most severe bug found this session. `CompiledPredicate::compile` returns `None` both for a genuine structural parse failure *and* for any real, well-formed SQL it simply doesn't implement yet — `LIKE`, `IN`, `BETWEEN`, and so on. Every caller treated that `None` as "keep everything" (`unwrap_or(true)`), a default meant to be a conservative fallback for the first case that's catastrophic for the second: `SELECT * FROM users WHERE name LIKE 'A%'` silently returned *every* row, `UPDATE ... WHERE name LIKE 'A%'` silently updated *every* row, and — verified empirically before this was fixed — `DELETE FROM users WHERE name LIKE 'A%'` silently deleted *every row in the table*, not just the matching one. The same conflation existed for `JOIN ... ON` (an unsupported condition silently ran as an unfiltered `CROSS JOIN`, not an error) and for the `HAVING` clause added earlier this session. Fixed everywhere at once: a *present* `WHERE`/`HAVING`/`ON` clause that fails to compile is now `DatabaseError::ExecutionError` before any row is touched; only a *genuinely absent* clause (no `WHERE` at all, a real `CROSS JOIN` with no `ON`) still means "match everything," which is correct. A separate, lower-severity default was left alone: a per-row/per-pair evaluation failure *after* successful compilation (a structural anomaly on one specific row, distinct from a `NULL` comparison — those are already handled correctly via `row_codec::Tri`) now excludes that row rather than keeping it, matching the same "don't guess, exclude" discipline already established for `NULL`. See [`execution::row_codec::CompiledPredicate`](src/execution/row_codec.rs) and the `Filter`/`Join`/`Aggregate`/`execute_update`/`execute_delete` arms of [`execution::executor`](src/execution/executor.rs).
- **An unsupported `UPDATE ... SET` expression, or `INSERT` value, is now a hard error too, not silent corruption**: the same root cause, one layer over — `CompiledAssignment::compile`'s fallback for a `SET` expression that isn't a plain literal, a bare column reference, or simple arithmetic (and, identically, `INSERT`'s own value parsing) used to parse the *whole expression text* as a literal for the target column's type, silently producing `Value::Null` on failure (`parse_value`'s existing convention). Verified empirically before the fix: `UPDATE users SET age = ABS(age)` silently wrote every matched row's `age` to `NULL`; worse, `UPDATE users SET name = UPPER(name)` silently wrote the literal, unparsed text `"UPPER(name)"` into every matched row's `name` column, because a `String` column accepted *any* raw text, quoted or not, as a valid literal; and `INSERT INTO items (id, price) VALUES (1, ABS(-9.99))` silently inserted `price = NULL`. Fixed with a new `parse_literal_checked`, stricter than `parse_value` in exactly the two ways that mattered: a `String` literal must actually be quoted, and anything that doesn't look like a real literal of the target type is a compile/insert-time error, not a silent `Null`. The one thing `parse_value`'s old ambiguity happened to get right — `SET col = NULL` / `INSERT ... VALUES (..., NULL, ...)` — still works, now because `NULL` is recognized explicitly, not because everything unparseable defaulted there. See [`execution::row_codec::CompiledAssignment`](src/execution/row_codec.rs) and `execution::executor::order_insert_row`.
- **A `WHERE`-clause literal that doesn't actually match its column's type is now a compile-time error, not a silent empty result**: a lower-severity sibling of the two bugs above, found by auditing the last remaining `parse_value` call site — `CompiledPredicate`'s own literal parsing (`row_codec::ExprParser::parse_atom`) used the same loose `parse_value`, so `WHERE age = 'thirty'` (an `Integer` column compared to a non-numeric literal) silently compiled fine and matched zero rows — not dangerous the way the fail-*open* bugs were (nothing was ever wrongly included), but still a real predicate mistake disguised as an unremarkable "no matches." A forgotten quote had the same problem the other direction: `WHERE name = Bob` (meant `'Bob'`) silently worked anyway, since a `String` column accepted any unquoted text as a valid literal. Both now use `parse_literal_checked` and fail to compile instead. `WHERE col = NULL` (non-standard SQL, but something this engine already recognized) still compiles and correctly matches nothing, same as before.
- **Prepared statements now support a `?`/`$N` placeholder in `HAVING`**: `execute_prepared`'s substitution walk over a `SELECT` plan's nodes never had an `Aggregate` arm, so `GROUP BY x HAVING COUNT(*) > ?` left the placeholder text in place, un-substituted, always hard-erroring rather than actually binding the parameter — and `prepare()`'s own placeholder counter had the identical gap, undercounting `param_count` for a HAVING-only placeholder. Fixing the substitution arm surfaced a second, real bug: `substitute_placeholders` rebuilds an expression by rejoining tokens with a single space between all of them, so a bound `HAVING COUNT(*) > ?` arrived as `"COUNT ( * ) > 1"`, not `"COUNT(*) > 1"` — which `aggregate::extract_calls` (the HAVING aggregate-call recognizer) failed to recognize as a call at all, since it required the `(` immediately after the function name with no whitespace tolerance. Fixed both together: the missing substitution/counting arms, and `extract_calls` now looking past optional whitespace before the `(` (matching `parse_aggregate`'s own already-whitespace-tolerant parsing).
- **`LIMIT`/`OFFSET` with anything other than a literal integer is now a parse error, not silently dropped**: found by checking whether prepared-statement placeholders work in `LIMIT` too — they don't (this engine has no real support for binding a parameter there), but the real bug is what happened when someone tried: `sql::parser::extract_limit`/`extract_offset` returned `None` both for "no `LIMIT` clause was written" and for "a `LIMIT` clause was written but isn't a plain integer literal," so `SELECT * FROM users LIMIT ?` silently returned *every row*, `LIMIT` discarded entirely — and, unlike every other placeholder location this engine already rejects outright, this happened even through `execute_sql` directly, with no `prepare()` involved at all. Fixed by making the two cases distinguishable: a present-but-non-literal `LIMIT`/`OFFSET` is now `DatabaseError::ParseError("LIMIT must be a literal non-negative integer, got: '?'")`, raised at parse time, before the statement reaches the binder or planner at all.
- **A type-mismatched `UPDATE ... SET` arithmetic expression is now an execution error, not silent corruption**: a second bug hiding in `CompiledAssignment`, one evaluation step past the compile-time fix above — `SET balance = balance + amount` compiles fine (a valid `<column-or-literal> <op> <column-or-literal>` shape), but *evaluating* it calls `arith`, which already returned `None` for a genuine type/operator mismatch (`Boolean + Integer`, `-`/`*`/`/` on two `String`s — only `+`, concatenation, is defined for strings) as well as for the legitimate cases (a `NULL` operand, or integer/float division by zero) — and `eval` collapsed all of them to the same silent `Value::Null`, indistinguishable from a correct result. Verified empirically before the fix: `UPDATE users SET age = age - name` (an `Integer` column minus a `String` column) succeeded and silently wrote `age = NULL`. Fixed by making `arith`/`CompiledAssignment::eval` return `Result` instead of collapsing to `Option`: a `NULL` operand or division by zero still legitimately evaluate to `Ok(Value::Null)` (matching this project's SQLite-compatibility target — division by zero is not an error in SQLite), but a genuine type/operator mismatch is now `Err`, surfaced by `execute_update` as `DatabaseError::ExecutionError` naming the offending column, aborting the transaction before anything is written. See [`execution::row_codec::CompiledAssignment::eval`](src/execution/row_codec.rs).
...
- **`UPDATE`/`DELETE` now use the primary-key *and* secondary indexes**: found while building [the monotonic-key ingestion proof of concept](#proof-of-concept-a-monotonic-key-ingestion-niche) — `UPDATE ... WHERE id = ?` and `DELETE ... WHERE id = ?` never consulted any index at all, unconditionally deserializing and predicate-checking *every* row in the table on every call, no matter how narrow `WHERE` was (`SELECT` already had this exact acceleration, `try_pk_index_scan`/`try_secondary_index_scan`, but it was never reused for writes). Fixed with `QueryExecutor::candidate_rows_for_write`, tried in order: exact primary-key equality (a direct point read), then any `CREATE INDEX`-ed column for `=`/`>`/`>=`/`<`/`<=` (via the same `secondary_index_candidates` `SELECT`'s own accelerated path now shares), falling back to the full scan for every other `WHERE` shape — narrow and safe by construction, since both callers already re-verify every candidate against the real predicate before writing anything, exactly as they did against a full scan's candidates before. Real, measured effect: 2,121 single-row `UPDATE`s by primary key went from 17.19s to 18.43ms — about 930x.
- **`LockManager::release_all` no longer scans every locked row in the database**: on every single commit or abort, it used to iterate *every currently-locked row across every transaction*, not just the calling transaction's own locks — real, measured cost: releasing one lock took 37.77ms with 200,000 unrelated rows locked by other transactions. Unlike `DynamicPGMIndex`'s bulk-insert bug, this doesn't scale with total data size, it scales with total *concurrent* lock count — real cost under exactly the high-concurrency, many-short-transactions shape a busy ingestion pipeline produces, paid on every commit. Fixed with `held_by`, a per-transaction index into the lock table (kept under the same lock, updated when a lock is granted) so `release_all` only ever touches the keys that transaction actually holds. Real, measured effect: the same scenario went from 37.77ms to 32.79µs — about 1,152x.
...
- [x] `UPDATE`/`DELETE` primary-key *and* secondary-index acceleration (`candidate_rows_for_write`) — a real, ~930x (PK) measured fix, found while building the monotonic-key ingestion proof of concept below
- [x] `LockManager::release_all` no longer scans the whole lock table — a real, ~1,152x measured fix, found while auditing for other silent performance bugs of the same shape
- [x] Unsupported `WHERE`/`HAVING`/`JOIN ON` clauses (`LIKE`, `IN`, `BETWEEN`, ...) now hard-error instead of silently matching every row — the most severe bug found this session, verified to have caused real data loss (`DELETE ... WHERE name LIKE 'A%'` deleted every row) before the fix
- [x] Unsupported `UPDATE ... SET` expressions and `INSERT` values (a function call, e.g.) now hard-error instead of silently writing `NULL` or, for a `String` column, the expression's own unparsed text — the same root cause one layer over, found by checking every sibling literal-parsing path for the identical pattern
- [x] A `WHERE`-clause literal that doesn't match its column's type, `LIMIT`/`OFFSET` with a non-literal expression (a placeholder, e.g.), and a `?`/`$N` in `HAVING` — three more real "parsed and silently discarded/mismatched" bugs found by systematically auditing every remaining literal-parsing and placeholder-substitution path for the same shape
- [x] A type-mismatched `UPDATE ... SET` arithmetic expression (`age - name`, a `String` column in an arithmetic op besides `+`) now hard-errors instead of silently writing `NULL` — the same fail-silent shape one evaluation step past `CompiledAssignment::compile`'s own already-fixed compile-time check, found by auditing `eval`'s `Option`-collapsing fallback next
